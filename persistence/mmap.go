...
// Advise advises the kernel about how the memory map will be used.
func (m *MappedFile) Advise(pattern mmap.AccessPattern) error {
	if m == nil || m.f == nil {
		return nil
	}
	return m.f.Advise(pattern)
}
...
func MmapReadOnly(path string) (*MappedFile, error) {
	...
	return &MappedFile{f: f, data: f.Bytes()}, nil
}
