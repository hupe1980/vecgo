...
func TestPayloadSurvivesFlushAndCompaction(t *testing.T) {
	...
	require.NoError(t, e.Insert(model.PKUint64(1), []float32{1, 0}, nil, []byte("p1")))
	...
	require.NoError(t, e2.Insert(model.PKUint64(2), []float32{0, 1}, nil, []byte("p2")))
	...
	require.NoError(t, e2.Compact(segmentsToCompact))
	...
	require.NoError(t, err)
	require.Len(t, res, 1)
	require.Equal(t, model.PKUint64(2), res[0].PK)
