...
void int4L2DistanceAvx(const float *__restrict__ query,
    ...
    const __m256 scale = _mm256_set1_ps(1.0f / 15.0f);
    ...
    for (; i <= dim - 16; i += 16) {
        ...
        // Unpack high nibbles (first values): shift right by 4
...
// int4L2DistancePrecomputedAvx uses precomputed lookup tables for faster distance.
// lookupTable is 16 * dim floats: lookupTable[i*16 + q] = dequantized value for dim i, quant q
void int4L2DistancePrecomputedAvx(const float *__restrict__ query,
                                   const uint8_t *__restrict__ code,
                                   int64_t dim,
...
// int4L2DistanceBatchAvx computes L2 distances for multiple INT4 codes.
// codes is a flattened array of n codes, each of size (dim+1)/2.
void int4L2DistanceBatchAvx(const float *__restrict__ query,
