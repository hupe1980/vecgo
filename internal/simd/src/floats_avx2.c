...
void dotProductAvx2(float *__restrict a, float *__restrict b, int64_t n, float *__restrict res)
{
...
// SquaredL2Avx2 computes squared L2 distance with 4-way accumulator unrolling + FMA.
// Processes 32 floats per iteration (4 accumulators × 8-wide AVX).
void squaredL2Avx2(float *__restrict vec1, float *__restrict vec2, int64_t n, float *__restrict result)
{
    // Four accumulators for better instruction-level parallelism
    __m256 sum1 = _mm256_setzero_ps();
    __m256 sum2 = _mm256_setzero_ps();
...
// PqAdcLookupAvx2 computes the sum of distances from a precomputed table.
// table: M x 256 floats (flattened)
// codes: M bytes
// m: number of subvectors
// result: pointer to float result
void pqAdcLookupAvx2(float *__restrict table, uint8_t *__restrict codes, int64_t m, float *__restrict result, const __m256i *offsets_ptr)
{
...
// ScaleAvx2 scales array a by (*scalar) in place.
...
void scaleAvx2(float *__restrict a, int64_t n, float *__restrict scalar)
{
    if (n <= 0) {
        return;
    }
