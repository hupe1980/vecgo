...
// prefetchVectorBatch uses x86-64 PREFETCHT0 instruction for efficient prefetching.
// PREFETCHT0 fetches data into all levels of the cache hierarchy (L1, L2, L3).
...
func prefetchVectorBatch(data []float32, dim, dataLen int, ids []model.RowID) {
	...
	if len(idsU32) > 0 {
		prefetchBatchAMD64(unsafe.Pointer(&data[0]), dim, len(idsU32), idsU32)
	}
}
