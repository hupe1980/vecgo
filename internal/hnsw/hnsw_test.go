...
func TestHNSW_DotProduct_DistanceOrdering(t *testing.T) {
	...
	h, err := New(func(o *Options) {
		o.Dimension = 3
		o.DistanceType = distance.MetricDot
		o.M = 8
		o.EF = 50
	})
	if !assert.NoError(t, err) {
	...
	brute, err := h.BruteSearch(ctx, query, 3, func(id model.RowID) bool { return true })
	if !assert.NoError(t, err) {
		return
	}
...
func runValidateInsertSearchCase(t *testing.T, ctx context.Context, tc TestCases) {
	...
	for _, qi := range queryIndices {
		bestCandidatesBrute, err := h.BruteSearch(ctx, vecs[qi], tc.K, func(id model.RowID) bool { return true })
		if err != nil {
